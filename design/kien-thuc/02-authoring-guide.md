...
Layout and permalink (`/kien-thuc/<slug>/`) come from `_config.yml` defaults.
...
All CTAs are one include: `{% raw %}{% include kien-thuc/cta.html variant="…" %}{% endraw %}`
(variants: `inline`, `note`, `card`, `banner`, `signature` — full parameter
list in the comment at the top of `_includes/kien-thuc/cta.html`).
...
```liquid
{% raw %}{% include kien-thuc/cta.html variant="card" heading="…" text="…" button="…" href="…" %}{% endraw %}
```

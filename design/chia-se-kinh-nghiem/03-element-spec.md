...
| Feed thumbnail | Natural aspect ratio inside the 1.91:1 … 4:5 window, cropped to the nearest bound outside it and centred on the image's attention point (`fx`/`fy` → `--kt-focus`); 2–3 pictures tile Facebook-style (two squares / one wide + two squares, 2px seams); hairline frame, `--paper-deep` while decoding; hover: 2% lean-in, 0.3s glide | Facebook's display model (owner's call, 2026-07-29): a picture in its own shape says what the post is about before the title; the saliency-centred crop keeps the part that earns the click. 2% is the owner-tuned answer to the pointer. *Which* picture(s) and *which* layout: the selection algorithm in `04-chon-anh-bia.md`. |
...
| **Opening block** (new) | `.kt-hoc-open`: `--paper-deep` slab, hairline, `corner($r-2)` — the separated-surface recipe an aside already uses. Label "TRƯỚC KHI ĐỌC" / "ÔN LẠI" in the `.kt-cta-label` register; then `.kt-hoc-guess`, Literata `read(16)` `--ink-2`, saying the answer is not here; stem Literata `read(17)` 620; options are `<label>`-wrapped radios, hairline, `corner($r-2)`, `read(16)`, ≥44px tall, border warms to `--indigo` on hover; the promise line is Literata `read(16)` `--ink-2`, revealed by `:has()` after any pick. Recall variant is a `<details>` instead | The open loop and the pretest in one element (06 §2). It sits after the lead figure so the preloaded LCP image keeps its place. **Every option replies the same**, and the block says so *before* the tap — a hook that leaks its answer stops working, and a silent option row with no warning reads as a broken control. Both halves are gated in `verify.mjs`. |
| **Closing block** (new) | Seam; h2 "Kiểm lại vài điểm" Literata `read(19)`, one step under the body's h2 — a closing device, not another chapter. **No lead sentence and no reset button** (both cut 2026-08-09 for space and tone). Two questions; `fieldset`/`legend` so a screen reader announces the stem with every option; per-option explanation on `--paper-deep` with a 3px left edge. Then `.kt-mang-ve`: label "ÁP DỤNG TẠI XƯỞNG CỦA BẠN" + one action, same slab recipe as the opener | Retrieval, then something to do on Monday (06 §3). **Everything below this block outranks it** — the share buttons most of all — so its height is a budget, not a preference: measured 1281px at three questions, 959px at two. The floor that never moves is the 44px option row. |
| **The verdict** (new) | Chosen row's border, the explanation's left edge, and the word opening the explanation all take `--ok` (green) or `--no` (red). The word is lifted from the explanation's own first sentence by the include, never generated | Owner's call, 2026-08-09, over a words-only first cut. Pair derived and measured in philosophy §3. **The words are the signal and the hue is the echo**: "Đúng." / "Chưa đúng." open every explanation, so a reader who cannot separate red from green loses nothing. |
...
| Đọc tiếp | Seam; head is h2 Literata `read(21)` 620 with two 44px arrow buttons pushed right. **A horizontal rail of six cards**, `scroll-snap-type: x mandatory`: card `flex: 0 0 44%` (76% at ≤40rem), 16:10 thumbnail on top, title Literata `read(17)`/(16) 600 indigo under it, meta Inter 14px. Arrows ship `hidden`, revealed by a small inline script, disabled at each end | Rebuilt 2026-08-09 (was three stacked rows with a small thumbnail). Same topic first, own series excluded, reading time on every card. **The card width is the design**: the next card is deliberately cut by the right edge, because a row that ends flush reads as finished. Six is argued in the include. The rail stays inside the reading measure. With no script it still swipes, and all six posts are in the HTML. |
